...
루프 속에서나 중간중간 데이터를 수집하고자 하는 용도 아닌 긴 코드의 결과를 저장하는 일반적인 변수라면, 대입은 단 한번만 이뤄져야한다.  
예외는 없다. 역할 하나당 하나의 변수다.
...
데이터 구조가 중요한 만큼 반드시 깔끔하게 관리해야한다. 다른 요소와 마찬가지로 개발을 진행할수록 데이터를 더 잘 이해하게 된다. 따라서 그 깊어진 이해를 프로그램에 반드시 반영해야 한다.
