...
> 당신이 비서 지원자 그룹을 인터뷰를 해야한다고 하자.  
> 당신의 목표는 비서 지원자 그룹 안에서 가장 좋은 비서를 뽑는 것이다.  
> 비서 후보들의 가치는 절대적 숫자로 판단 할수 없지만 상대적으로는 쉽게 판단할 수 있다.  
> 철저히 랜덤한 순서로 지원자를 인터뷰 하게된다.  
...
두번째 지원자는 1/2 확률로 앞사람 보다 더 나을수도 더 못날 수도 있다.  
세번째 지원자는 앞사람들보다 더 나을 확률이 1/3이다.  
... 7번째 지원자는 1/7 확률로 지금까지의 최고의 비서지원자가 된다.

뒷차례 지원자가 될 수록 앞사람들을 넘은 최고의 후보가 될 확률은 적어지게 된다.  
즉, 의식에 따라 지원자를 넘기다 보면 점점 만족스러운 지원자를 찾기는 더 힘들어 지게 된다는 이야기이다.
...
하지만 이 방법이 의미있는건 지원자의 숫자가 천명이든 만명이 이든 혹은 그이상이 되어도 언제나 37%의 확률로 최고의 후보를 뽑을 수 있다는 것이다.
...
현실에서 Optimal Stopping를 이용하여 집을 팔때 자주 하는 문제가 있다. 바로 처음에 각 단계별 한계점을 정했지만 도중에 변경한다는 점이다. 시장가격과 대출이자를 도대로 한계점을 설정하였지만, 팔지 못했을 경우 걸리는 불어나는 이자와 시간때문에 한계점에 못미치는 가격에 거래가 많이 된다. 드물게 오는 고객에 조급해하지 않고 수학을 믿고 거래해야 수학적으로 이점을 취할 수 있다.
...
50%의 점유율이라면 반은 차가 있고 반은 없기 때문에 평균적으로 1개는 지나쳐야 한다. 70%에서 80%까지는 천천히 올라가다 점유율이 95%가 넘어가면 급격하게 지나쳐야할 개수가 늘어난다. 점유율의 99%가 되어있다면 평균적으로 69나 지나쳐야하고 99.9%에 도달했다면 평균적으로 693%나 지나쳐야 주차공간을 발견할 수 있다.
